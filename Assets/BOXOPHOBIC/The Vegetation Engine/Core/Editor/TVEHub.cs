...
namespace TheVegetationEngine
{
    public class TVEHub : EditorWindow
    {
        ...
        bool needsUpgradeForHDRP = false;
        ...
        void OnEnable()
        {
            ...
            allMeshGUIDs = AssetDatabase.FindAssets("TVE Mesh", null);
            ...
            for (int i = 0; i < allMaterialGUIDs.Length; i++)
            {
                ...
                if (path.Contains("TVE Material") && !path.Contains("The Vegetation Engine"))
                {
            ...
            for (int i = 0; i < allMeshGUIDs.Length; i++)
            {
                ...
                if (path.Contains("TVE Mesh") && !path.Contains("The Vegetation Engine"))
                {
                    userMeshPath = path;
                    break;
                }
            ...
            if (userMaterialPath != "")
            {
                ...
                {
                    if (material.GetInt("_IsVersion") < 650)
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (showAdvancedSettings)
                        {
                            ...
                            EditorGUILayout.HelpBox("Use the Validate options below if materials or meshes converted with the Vegetation Engine prior to version 6.5.0 were imported in the project!", MessageType.Info, true);
        ...
        void InstallAsset()
        {
            ...
            SettingsUtils.SaveSettingsData(assetFolder + "/Core/Editor/Validator.asset", "True");
        ...
        void UpgradeMaterials()
        {
            ...
            foreach (var asset in allMaterialGUIDs)
            {
                ...
                EditorUtility.DisplayProgressBar("The Vegetatin Engine", "Processing " + Path.GetFileName(path), (float)count * (1.0f / (float)length));
            }
        ...
        void UpgradeMeshes()
        {
            int length = allMeshGUIDs.Length;
            int count = 0;
