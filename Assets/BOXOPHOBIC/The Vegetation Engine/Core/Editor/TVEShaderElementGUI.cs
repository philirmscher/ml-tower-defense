...
public class TVEShaderElementGUI : ShaderGUI
{
    ...
    void DrawDynamicInspector(Material material, MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        else
        {
            for (int i = 0; i < props.Length; i++)
            {
                ...
                {
                    if (material.GetInt("_ElementInteractionMode") == 12 && prop.name == "_MotionPower")
                        continue;
                    if (material.GetInt("_ElementInteractionMode") == 13 && prop.name == "_MotionPower")
                        continue;
                }

                if (material.HasProperty("_ElementRaycastMode"))
                {
                    if (material.GetInt("_ElementRaycastMode") == 0 && prop.name == "_RaycastMessage")
                        continue;
    ...
    {
        if (shader.name.Contains("Interaction") || shader.name.Contains("Orientation"))
        {
