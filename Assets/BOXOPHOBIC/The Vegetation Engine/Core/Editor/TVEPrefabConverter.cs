...
namespace TheVegetationEngine
{
    ...
    public class TVEPrefabConverter : EditorWindow
    {
            ...
            if (presetIndex != 0)
            {
                ...
                if (showAdvancedSettings)
                {
                    ...
                    if (outputMeshIndex == OutputMesh.Default)
                    {
                        ...
                        sourceMotion2 = StyledSourcePopup("Squash Mask", "Motion mask used for squash animations. Stored in UV0.Z as a packed mask.", sourceMotion2, SourceMaskEnum);
                        optionMotion2 = StyledMaskOptionEnum("Squash Mask", "", sourceMotion2, optionMotion2, false);
                        textureMotion2 = StyledTexture("Squash Mask", "", sourceMotion2, textureMotion2);
                        actionMotion2 = StyledActionOptionEnum("Squash Mask", "", sourceMotion2, actionMotion2, true);
                    ...
                    if (outputMeshIndex == OutputMesh.Polygonal)
                    {
                        ...
                        sourceMotion3 = StyledSourcePopup("Motion Flutter", "Motion mask used for flutter animation. Stored in UV0.Z as a packed mask.", sourceMotion3, SourceMaskEnum);
        ...
        void CreateMeshInstances()
        {
            ...
            for (int i = 0; i < meshFiltersInPrefab.Count; i++)
            {
                if (meshesInPrefab[i] != null)
                {
                    ...
                    meshInstance.name = meshesInPrefab[i].name + " " + i.ToString("X2") + " (" + outputSuffix + " Mesh)";
        ...
        void CreateColliderInstances()
        {
            ...
            for (int i = 0; i < meshCollidersInPrefab.Count; i++)
            {
                if (collidersInPrefab[i] != null)
                {
                    ...
                    colliderInstance.name = collidersInPrefab[i].name + " " + i.ToString("X2") + " (" + outputSuffix + " Mesh)";
        ...
        void SaveMesh(Mesh mesh)
        {
            ...
            string savePath = "/" + meshName + ".asset";
        ...
        List<Vector4> GetCoordData(Mesh mesh, int source, int option)
        {
            ...
            if (meshCoord.Count == 0)
            {
                // If no coords are found, set automatic LM coords
